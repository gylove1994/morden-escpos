...
export function CanvasPanel() {
  const t = useTranslations('CanvasPanel');
    ...
    <Tabs value={viewMode} onValueChange={value => setViewMode(value as typeof viewMode)} className="h-full min-h-0 w-full flex-col gap-0">
      <div className="flex h-10 shrink-0 items-center justify-between border-b bg-background/90 px-3">
        <TabsList className="h-8">
          <TabsTrigger value="edit" className="px-3 text-xs">{t('edit')}</TabsTrigger>
          <TabsTrigger value="printPreview" className="px-3 text-xs">{t('preview')}</TabsTrigger>
        </TabsList>
        <span className="text-[11px] text-muted-foreground">
          ...
          mm ·
          {' '}
          {t('commandCount', { count: commandCount })}
        </span>
      </div>
