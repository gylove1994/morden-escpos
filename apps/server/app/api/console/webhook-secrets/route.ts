...
/**
 * List webhook signing secrets for the active Organization.
 * Any signed-in org member MAY list; plaintext secrets are never returned.
 */
export async function GET(request: Request) {
  ...
  const webhookSecrets = await listWebhookSigningSecrets(
    consoleSession.organization.id,
  );
  return Response.json({ webhookSecrets });
}

/**
 * Create a webhook signing secret and return the plaintext once.
...
export async function POST(request: Request) {
  ...
  if (!canManageIntegratorAuth(consoleSession.role)) {
    return Response.json(
      {
        error: 'forbidden',
        message: 'owner or admin role required to create a webhook signing secret',
      },
      { status: 403 },
    );
  }
  ...
  const created = await createWebhookSigningSecret({
