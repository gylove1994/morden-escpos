...
function removeObjectSchemaEntry(
  ...
  if (!isObjectSchema(schema) || !schema[schemaProperty])
    return schema;
  ...
  if (!entries)
    return newSchema;
  const { [propertyName]: _, ...remainingProps } = entries;
...
export function createFieldSchema(field: NewField): JsonSchema {
  ...
  return {
    type,
    description: description || undefined,
    ...(validation !== undefined && isObjectSchema(validation) ? validation : {}),
...
export function validateFieldName(name: string): boolean {
  ...
  const validNamePattern = /^[a-z_$][\w$]*$/i;
