...
import { createSchemaFromJson } from '../src/lib/schema-inference';
...
describe('schema Inference', () => {
  ...
  it('should infer schema for object types', () => {
    const json = {
      person: {
        name: 'John',
        age: 30,
  ...
  it('should infer schema for array types', () => {
    const json = {
      numbers: [1, 2, 3],
      mixed: [1, 'two', true],
      empty: [],
  ...
  it('should infer schema for array of objects with different properties', () => {
    const json = {
      users: [
        {
          name: 'John',
  ...
  it('should handle mixed types for the same property using oneOf', () => {
    const json = {
      data: [
        { key: 'A', value: 100 },
        { key: 'B', value: 'hello' },
  ...
  it('should correctly identify required fields when properties are missing', () => {
    const json = {
      records: [
  ...
  it('should detect enums for strings in larger arrays', () => {
    const json = {
  ...
  it('should detect enums for numbers in larger arrays', () => {
    const json = {
      sensors: Array.from({ length: 15 })
        .fill(null)
        .map((_, i) => ({
          sensorId: `sensor_${i}`,
  ...
  it('should NOT detect enums for unique IDs or insufficient data', () => {
    const json = {
      shortList: [{ status: 'A' }, { status: 'B' }, { status: 'C' }],
      uniqueIds: Array.from({ length: 12 })
        .fill(null)
  ...
  it('should detect coordinate arrays [lat, lon]', () => {
    const json = {
      locations: [
  ...
  it('should detect coordinate arrays [lat, lon, alt]', {
  ...
  it('should NOT detect coordinates if structure varies or type is wrong', {
  ...
  it('should detect timestamp integers', () => {
    const now = Date.now();
    const json = {
      logs: [
  ...
  it('should NOT detect timestamp for non-integer or out-of-range values', () => {
    const json = {
      data: [
  ...
  it('should handle arrays where items merge into a single object type', () => {
    const json = {
      values: [
        { common: 'yes', typeA: 1 },
  ...
  it('should handle primitive root input (string)', () => {
  ...
  it('should handle primitive root input (number)', () => {
    const json = 12345;
    const schema = createSchemaFromJson(json);
    assert.strictEqual(schema.type, 'object');
    assert.ok(schema.properties.value, 'Should have a \'value\' property');
  ...
  it('should handle primitive root input (boolean)', () => {
    const json = false;
    const schema = createSchemaFromJson(json);
    assert.strictEqual(schema.type, 'object');
    assert.ok(schema.properties.value, 'Should have a \'value\' property');
  ...
  it('should handle array root input', () => {
  ...
  it('should handle empty array root input', () => {
    const json = [];
    const schema = createSchemaFromJson(json);
    assert.strictEqual(schema.type, 'array');
  ...
  it('should handle null root input', () => {
    const json = null;
    const schema = createSchemaFromJson(json);
