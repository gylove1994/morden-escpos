...
import type { OAuthAuthorizationRequest } from '@better-auth-ui/core/plugins';
import type { ListDeviceSession, OAuthProviderMultiSessionAuthClient } from '@better-auth-ui/react';
import {
...
export interface OAuthSelectAccountProps {
  className?: string
}

function interpolateClient(template: string, clientName: string) {
  return template.replace('{{client}}', clientName);
}
