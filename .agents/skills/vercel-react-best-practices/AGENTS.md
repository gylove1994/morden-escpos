...
Vercel Engineering (adapted for Tellus client-side React)  
July 2026

> **Note:** Server-side, RSC, Next.js, and hydration rules have been removed.
...
In API route handlers, start independent operations immediately, even if you don't await them yet.
...
**Correct — Vite `optimizeDeps` for barrel-heavy packages:**
...
Prefer explicit maps or literal paths so the set of reachable files stays narrow and predictable. This is the same rule whether you are choosing modules with `import()` or reading files in build code.
