...
- Use `fn()` from `storybook/test` for action args instead of `action()` — `fn()` enables `toHaveBeenCalled*` assertions.
...
- This repo **keeps an explicit `title`** with the hierarchy `ui/radix/<Component>` or `ui/patterns/<Component>`; do not drop the title to let Storybook auto-generate it.
