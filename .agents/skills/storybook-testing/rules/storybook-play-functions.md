...
**Tellus repo notes:**
- Import types from `@storybook/react-vite` (not `@storybook/react` as shown in the examples).
- Regression stories carry `tags: ["!dev", "!autodocs"]`, export names start with `Should`, and `name` is a full `"when …, should …"` sentence; split interactions with `step()`.
- Query portal-rendered content (Dialog/Popover, etc.) via `within(canvasElement.ownerDocument.body)`.
- This repo has no Vitest/CI runner for play tests; verify via the local Storybook Interactions panel plus `build-storybook`.
